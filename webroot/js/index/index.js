...
function login (){
    ...
    if(user==""||psd==""){
        ...
        var datajson = {name: user, password: psd,loginTime:loginTime};
        $.ajax({
            type: "post",
            url: "/api/index/login",
            dataType: "json",
...
function regist(){
    ...
    if (inputCode.length <= 0) {
        ...
        if(user==""||psd==""||email==""||phone==""){
            ...
            datajson={name:user,password:psd,email:email,phone:phone,userImg:imgUrl,registTime:registTime};
            $.ajax({
                type: "post",
...
function otherLogin(){
    ...
    QC.Login({
        ...
        if( getCookie('loginStatu')!="qq" || getCookie('token')=='') {
            //登录成功
            var loginTime = createTime();
            var qqUserInfo = {nickname: reqData.nickname, userava:reqData.figureurl_qq_2,loginAcount: "qq", loginTime: loginTime};
            $.ajax({
                type: "post",
