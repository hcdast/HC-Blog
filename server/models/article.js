...
var ArticleSchema = new Schema({
    ...
    title: { type: String },
    content: { type: String, },
    type: {type: String},  //（0：政治知事:1：国防军事、2：经济论坛:3：文化先锋、4：科学探索、5：体育中心）
    author: { type: String }, 
    imgurl: { type: String },
    pageurl: {type: String },
    time: {type: String },
...
var addArticle = function (data,callback) { 
    ...
    newArticle.save(function (err) {
        if(err){
            console.log(err);
             callback({statu:"err",msg:"发表失败,请稍后重试。。。"});
        }else{
...
// 获取博主文章个数
var getArticleCountByName = function(data,callback) {
    model.count(data,function (err,count) {
        if(!err){
            callback({statu:"success",msg:"获取数据成功！",count:count});
        }else{
            callback({statu:"err",msg:"获取博主文章数据失败 ！"});
        }
     });
}
// 点击排行
...
// 获取最新文章
var getNewArticle = function (callback) {
    model.find({},{title:1,pageurl:1},function (err,docs) {
        if(!err){
             if(docs.length>0){
                 callback({statu:"success",msg:"获取数据成功！",data:docs});
...
// 获取最热文章
var getHotArticle = function (callback){
    model.find({},{title:1,pageurl:1},function (err,docs) {
        if(!err){
             if(docs.length>0){
                 callback({statu:"success",msg:"获取数据成功！",data:docs});
...
// 通过id查找数据
var getArticleById = function (data,options,callback) {
...
module.exports ={
    ...
    getArticleCountByName:getArticleCountByName,
    clickRank:clickRank,
