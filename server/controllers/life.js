...
function getData(req, res, next) {
    ...
    life.getData(data, function (result) {
        ...
        redisClient.getRedis(token, function (result) {
            if (result.statu == "success") {
                if (parseInt(userinfo.identity) < 3) {
                    ...
                    userinfo.pageurl = "lifePages/" + userinfo._id + ".html";

                    var readpath = "../webroot/lifePages/model.html";
                    var writepath = "../webroot/lifePages/" + userinfo._id + ".html";
                    copyfile.copyFile(readpath, writepath, function (result) {
                        if (result.statu == "success") {
                            var path = "../webroot/images/life/" + userinfo._id + ".png";
                            ...
                            fs.writeFile(path, dataBuffer, function (err) { //用fs写入文件
                                if (err) {
                                    ...
                                    console.log('图片保存成功！');
                                    userinfo.imgurl = "images/life/" + userinfo._id + ".png";
                                    life.addLife(userinfo, function (result) {
                                        if (result.statu == "success") {
                                            res.end(JSON.stringify({
                                                statu: result.statu,
                                                msg: result.msg
...
function clickRank(req, res, next) {
    ...
    life.clickRank(function (result) {
        if (result.statu == "success") {
            var data = result.data;
            if (data.length > 0) {
                res.end(JSON.stringify({
...
function newComments(req,res,next) {
    ...
    var arg = url.parse(req.url, true).query;
    var id = arg.id;
    life.newComments({_id:id},function (result) {
        if (result.statu == "success") {
            res.end(JSON.stringify({
                statu: "success",
                msg: result.msg,
...
function updateClikNum(req, res, next) {
    ...
    var arg = url.parse(req.url, true).query;
    var id = arg.id;
    life.updateClikNum({_id:id},function (result) {
        if (result.statu == "success") {
            res.end(JSON.stringify({
                statu: "success",
                msg: result.msg,
...
function getLifeById(req,res,next) {
    ...
    life.getAllData(function (result) { 
        if(result.statu=="success"){
            var data = result.data;
            var predata={};
            var netdata={};
            ...
            async.whilst(
                ...
                function (err) {
                    life.getLifeById({_id:id},options, function (result) {
                        if (result.statu == "success") {
                            predata={ 
    ...
    req.addListener("end", function () {
        ...
        redisClient.getRedis(token, function (result) {
            if (result.statu == "success") {
                ...
                life.addCommentById({_id:id},userinfo,function (result) {
                    if(result.statu=="success"){
                        res.end(JSON.stringify({
                            statu: "success",
                            msg: result.msg
...
function userAct(req,res,next) {
    ...
    req.addListener("end", function () {
        ...
        if(act=="praise"){
            life.userAct({_id:id},{$inc:{praise:1}},function (result) { 
                res.end(JSON.stringify({
                    statu: result.statu,
                    msg: result.msg
                }));
            ...
            life.userAct({_id:id},{$inc:{pit:1}},function (result) { 
                res.end(JSON.stringify({
                    statu: result.statu,
                    msg: result.msg
                }));
            ...
            life.userAct({_id:id},{$inc:{report:1}},function (result) { 
                res.end(JSON.stringify({
                    statu: result.statu,
                    msg: result.msg
                }));
...
function getTypeData(req,res,next) {
    ...
    var arg = url.parse(req.url, true).query;
    var type = arg.type;
    life.getTypeData({type:type},function (result) {
        if (result.statu == "success") {
            res.end(JSON.stringify({
                statu: "success",
                msg: result.msg,
...
function remove(req, res, next) {
    ...
    var htmlurl = "../webroot/lifePages/"+id+".html";
    var imgurl = "../webroot/images/life/"+id+".png";
